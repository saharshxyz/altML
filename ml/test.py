...
def runModel(img_tuple):
	...
	captions_array = []
	for index in range(len(img_tuple)):
		imgURL= img_tuple[index]
		urllib.request.urlretrieve(imgURL, r"C:\Users\Jonny\Image-Caption-Generator\test_data\image"+str(index)+".jpg")
	...
	for image_file in os.listdir(config['test_data_path']):
		...
		if(image_file.split('.')[1]=='jpg' or image_file.split('.')[1]=='jpeg'):
			...
			_ = ax.set_title("BEAM Search with k={}n{}".format(config['beam_search_k'],caption),fontdict={'fontsize': '20','fontweight' : '40'})
			plt.savefig(config['test_data_path']+'output--'+image_file)
			'''
			captions_array.append(caption)
			os.remove(r"C:\Users\Jonny\Image-Caption-Generator\test_data"+"\\"+image_file)
	return captions_array
print(runModel(config['test_data_tuple']))
print(runModel(config['test_data_tuple']))
